...
exports.createModel = function(type, options) {
  ...
  Model.prototype = {
    ...
    validate: function() {
      ...
      Model.validators.forEach(function(validator) {
        validator(model, failures);
      });

      if (failures.length) {
        return this.error("Validations failed.", failures);
      }
    ...
    error: function(message, validationFailures) {
      var error = new Error(message);

      if (validationFailures) {
        error.failures = validationFailures;
      }
  ...
  Model.validators = [];
  Array.prototype.push.apply(Model.validators, require('./validators'));
